...
function usesDynamicIdScope(value) {
  ...
  return typeof openapi2 === "string" && /^3\.1(?:\.|$)/.test(openapi2);
}
...
function getSchemaIdMode(value, inheritedLegacyId = false) {
  ...
  const openapi2 = value.openapi;
  if (typeof openapi2 === "string" && /^3\.1(?:\.|$)/.test(openapi2)) {
    return false;
  }
  return inheritedLegacyId;
}
...
var $Refs = (_class11 = class {
   ...
   */
  get(path, options) {
    return _optionalChain([this, 'access', _21 => _21._resolve, 'call', _22 => _22(path, "", options), 'optionalAccess', _23 => _23.value]);
  }
  /**
...
function tagNameFull(rawTag, tagHandlers) {
  ...
  const prefix = _nullishCoalesce(_nullishCoalesce(_optionalChain([tagHandlers, 'optionalAccess', _24 => _24[handle]]), () => ( DEFAULT_TAG_HANDLERS[handle])), () => ( handle));
...
function addMappingValue(state, frame, key, value, tag) {
  ...
  if (!state.json && frame.tag.has(frame.value, key) && !_optionalChain([frame, 'access', _25 => _25.overridable, 'optionalAccess', _26 => _26.has, 'call', _27 => _27(key)])) throwError$1(state, "duplicated mapping key");
  const err = frame.tag.addPair(frame.value, key, value);
  if (err) throwError$1(state, err);
  _optionalChain([frame, 'access', _28 => _28.overridable, 'optionalAccess', _29 => _29.delete, 'call', _30 => _30(key)]);
}
...
function parseNode(state, parentIndent, nodeContext, allowToSeek, allowCompact, allowPropertyMapping = true) {
  ...
  if (indentStatus === 1) while (true) {
    ...
    if (atNewLine && allowBlockStyles && (props.tagStart !== NO_RANGE$1 || props.anchorStart !== NO_RANGE$1) && (ch === 33 || ch === 38)) {
      ...
      if (readBlockMapping(state, state.position - state.lineStart, flowIndent, props) && _optionalChain([state, 'access', _31 => _31.events, 'access', _32 => _32[fallbackState.eventsLength], 'optionalAccess', _33 => _33.type]) === 3) {
        state.depth--;
        return true;
      }
  ...
  if (indentStatus === 1 || nodeContext === CONTEXT_BLOCK_OUT) {
    ...
    if (indentStatus === 1) if (allowBlockCollections && (readBlockSequence(state, blockIndent, props) || readBlockMapping(state, blockIndent, flowIndent, props)) || readFlowCollection(state, flowIndent, props)) hasContent = true;
      ...
      if (propertyStart !== null && allowPropertyMapping && allowBlockStyles && !allowBlockCollections && ch !== 124 && ch !== 62) {
        ...
        if (readBlockMapping(state, propertyIndent, flowIndent, emptyProperties()) && _optionalChain([state, 'access', _34 => _34.events, 'access', _35 => _35[fallbackState.eventsLength], 'optionalAccess', _36 => _36.type]) === 3) hasContent = true;
...
function readDocument(state) {
  ...
  if (_optionalChain([documentEvent, 'optionalAccess', _37 => _37.type]) === 1) documentEvent.explicitEnd = explicitEnd;
  addPopEvent(state);
    ...
    if (typeof data === "string") {
      try {
        ...
        try {
          ...
          throw new ParserError(_optionalChain([e, 'optionalAccess', _38 => _38.message]) || "Parser Error", file.url);
        }
      }
    } else {
      return data;
...
Too many redirects: 
  ${redirects.join(" \n  ")}`);
    ...
    _optionalChain([pendingResponse, 'optionalAccess', _39 => _39.cancelTimeout, 'call', _40 => _40()]);
  }
}
async function get(u, httpOptions, resolvedAddresses) {
  ...
  const dispatcher = _optionalChain([pinnedTransport, 'optionalAccess', _41 => _41.dispatcher]);
  let controller;
  let timeoutId;
...
function resolveExternal(parser, options) {
  if (!_optionalChain([options, 'access', _42 => _42.resolve, 'optionalAccess', _43 => _43.external])) {
  ...
  try {
    ...
    const promises = crawl(parser.schema, parser.$refs._root$Ref.path + "#", rootScopeBase, parser.$refs._root$Ref.dynamicIdScope, parser.$refs._root$Ref.legacyIdScope, parser.$refs, options);
    return Promise.all(promises);
  } catch (e) {
    return Promise.reject(e);
  }
}
function crawl(obj, path, scopeBase, dynamicIdScope, legacyIdScope, $refs, options, seen, external) {
  seen ||= /* @__PURE__ */ new Set();
  let promises = [];
  if (obj && typeof obj === "object" && !ArrayBuffer.isView(obj) && !seen.has(obj)) {
    ...
    for (const key of keys) {
      ...
      promises = promises.concat(crawl(value, keyPath, childScopeBase, dynamicIdScope, childLegacyIdScope, $refs, options, seen, external));
    }
  }
  return promises;
}
async function resolve$Ref($ref, path, scopeBase, dynamicIdScope, $refs, options) {
  const shouldResolveOnCwd = _optionalChain([options, 'access', _44 => _44.dereference, 'optionalAccess', _45 => _45.externalReferenceResolution]) === "root";
  ...
  try {
    ...
    const parsedDynamicIdScope = _nullishCoalesce(_optionalChain([parsedRef, 'optionalAccess', _46 => _46.dynamicIdScope]), () => ( false));
    const parsedLegacyIdScope = _nullishCoalesce(_optionalChain([parsedRef, 'optionalAccess', _47 => _47.legacyIdScope]), () => ( false));
    const parsedScopeBase = parsedDynamicIdScope ? getSchemaBasePath(withoutHash, result, parsedLegacyIdScope) : withoutHash;
    const promises = crawl(result, withoutHash + "#", parsedScopeBase, parsedDynamicIdScope, parsedLegacyIdScope, $refs, options, /* @__PURE__ */ new Set(), true);
    return Promise.all(promises);
  } catch (err) {
    if (!_optionalChain([options, 'optionalAccess', _48 => _48.continueOnError]) || !isHandledError(err)) {
      throw err;
    }
...
function bundle(parser, options) {
  ...
  crawl2(parser, "schema", parser.$refs._root$Ref.path + "#", rootScopeBase, parser.$refs._root$Ref.dynamicIdScope, parser.$refs._root$Ref.legacyIdScope, false, "#", 0, inventory, parser.$refs, options, embeddedResourcePaths, /* @__PURE__ */ new Set());
...
function crawl2(parent, key, path, scopeBase, dynamicIdScope, legacyIdScope, nestedIdScope, pathFromRoot, indirections, inventory, $refs, options, embeddedResourcePaths, seen) {
    ...
    if (ref_default.isAllowed$Ref(obj, options)) {
      ...
      for (const key2 of keys) {
        ...
        if (ref_default.isAllowed$Ref(value, options)) {
          ...
          crawl2(obj, key2, keyPath, childScopeBase, dynamicIdScope, childLegacyIdScope, childNestedIdScope, keyPathFromRoot, indirections, inventory, $refs, options, childEmbeddedResourcePaths, seen);
        }
        if (value && typeof value === "object" && !Array.isArray(value)) {
          if ("$ref" in value) {
            _optionalChain([bundleOptions, 'optionalAccess', _49 => _49.onBundle, 'optionalCall', _50 => _50(value["$ref"], obj[key2], obj, key2)]);
          }
        }
      }
    }
...
function inventory$Ref($refParent, $refKey, path, scopeBase, dynamicIdScope, nestedIdScope, pathFromRoot, indirections, inventory, $refs, options, embeddedResourcePaths, seen) {
  ...
  const shouldResolveOnCwd = ref_default.isExternal$Ref($ref) && _optionalChain([options, 'access', _51 => _51.dereference, 'optionalAccess', _52 => _52.externalReferenceResolution]) === "root";
  const resolutionBase = shouldResolveOnCwd ? cwd() : dynamicIdScope ? scopeBase : path;
  ...
  if (!existingEntry || external) {
    ...
    crawl2(pointer.value, null, pointer.path, resolvedScopeBase, pointer.$ref.dynamicIdScope, pointer.legacyIdScope, resolvedNestedIdScope, pathFromRoot, indirections + 1, inventory, $refs, options, resolvedEmbeddedResourcePaths, seen);
  }
}
...
function dereference(parser, options) {
  ...
  const rootScopeBase = parser.$refs._root$Ref.dynamicIdScope ? getSchemaBasePath(parser.$refs._root$Ref.path, parser.schema, parser.$refs._root$Ref.legacyIdScope) : parser.$refs._root$Ref.path;
  const dereferenced = crawl3(parser.schema, parser.$refs._root$Ref.path, rootScopeBase, parser.$refs._root$Ref.dynamicIdScope, parser.$refs._root$Ref.legacyIdScope, "#", /* @__PURE__ */ new Set(), /* @__PURE__ */ new Set(), /* @__PURE__ */ new Map(), parser.$refs, options, start, 0);
  parser.$refs.circular = dereferenced.circular;
  parser.schema = dereferenced.value;
}
function crawl3(obj, path, scopeBase, dynamicIdScope, legacyIdScope, pathFromRoot, parents, processedObjects, dereferencedCache, $refs, options, startTime, depth) {
  let dereferenced;
  const result = {
    value: obj,
    circular: false
  ...
  if (_optionalChain([derefOptions, 'optionalAccess', _53 => _53.circular]) === "ignore" || !processedObjects.has(obj)) {
        ...
        for (const key of Object.keys(obj)) {
          ...
          if (ref_default.isAllowed$Ref(value, options)) {
            ...
            if (obj[key] !== dereferenced.value) {
              ...
              if (_optionalChain([derefOptions, 'optionalAccess', _54 => _54.preservedProperties])) {
                if (typeof obj[key] === "object" && !Array.isArray(obj[key])) {
                  _optionalChain([derefOptions, 'optionalAccess', _55 => _55.preservedProperties, 'access', _56 => _56.forEach, 'call', _57 => _57((prop) => {
                    if (prop in obj[key]) {
              ...
              if (_optionalChain([derefOptions, 'optionalAccess', _58 => _58.cloneReferences]) && !circular && assignedValue && typeof assignedValue === "object") {
                assignedValue = structuredClone(assignedValue);
              }
              obj[key] = assignedValue;
              if (_optionalChain([derefOptions, 'optionalAccess', _59 => _59.preservedProperties])) {
              ...
              _optionalChain([derefOptions, 'optionalAccess', _60 => _60.onDereference, 'optionalCall', _61 => _61(value.$ref, obj[key], obj, key)]);
            }
          } else {
            if (!parents.has(value)) {
              dereferenced = crawl3(value, keyPath, childScopeBase, dynamicIdScope, childLegacyIdScope, keyPathFromRoot, parents, processedObjects, dereferencedCache, $refs, options, startTime, depth + 1);
              circular = dereferenced.circular;
              if (obj[key] !== dereferenced.value) {
...
function dereference$Ref($ref, path, scopeBase, dynamicIdScope, pathFromRoot, parents, processedObjects, dereferencedCache, $refs, options, startTime, depth) {
  ...
  const shouldResolveOnCwd = isExternalRef && _optionalChain([options, 'optionalAccess', _62 => _62.dereference, 'optionalAccess', _63 => _63.externalReferenceResolution]) === "root";
  const resolutionBase = shouldResolveOnCwd ? cwd() : dynamicIdScope ? scopeBase : path;
  const $refPath = resolve(resolutionBase, $ref.$ref);
  ...
  if (!circular) {
    ...
    const dereferenced = crawl3(dereferencedValue, pointer.path, dereferencedScopeBase, pointer.$ref.dynamicIdScope, pointer.legacyIdScope, pathFromRoot, parents, processedObjects, dereferencedCache, $refs, options, startTime, depth + 1);
    circular = dereferenced.circular;
    dereferencedValue = dereferenced.value;
  }
  if (circular && !directCircular && _optionalChain([options, 'access', _64 => _64.dereference, 'optionalAccess', _65 => _65.circular]) === "ignore") {
...
function foundCircularReference(keyPath, $refs, options) {
  $refs.circular = true;
  _optionalChain([options, 'optionalAccess', _66 => _66.dereference, 'optionalAccess', _67 => _67.onCircular, 'optionalCall', _68 => _68(keyPath)]);
...
function convertFileResolverOption(options, defaultFileResolver) {
  if (!_optionalChain([options, 'optionalAccess', _69 => _69.resolve]) || !("file" in options.resolve)) {
    return false;
  }
  ...
  return options.resolve.file;
}
function convertOptionsForParser(options, opts = {}) {
  const parserOptions = getJsonSchemaRefParserDefaultOptions();
  const fileOption = opts.allowFileResolution && !(_optionalChain([options, 'optionalAccess', _70 => _70.resolve]) && "file" in options.resolve) ? parserOptions.resolve.file : convertFileResolverOption(options, parserOptions.resolve.file);
  return {
    ...parserOptions,
    dereference: {
      ...parserOptions.dereference,
      circular: _optionalChain([options, 'optionalAccess', _71 => _71.dereference]) && "circular" in options.dereference ? options.dereference.circular : parserOptions.dereference.circular,
      onCircular: _optionalChain([options, 'optionalAccess', _72 => _72.dereference, 'optionalAccess', _73 => _73.onCircular]) || parserOptions.dereference.onCircular,
      onDereference: _optionalChain([options, 'optionalAccess', _74 => _74.dereference, 'optionalAccess', _75 => _75.onDereference]) || parserOptions.dereference.onDereference,
    ...
    resolve: {
      ...
      external: _optionalChain([options, 'optionalAccess', _76 => _76.resolve]) && "external" in options.resolve ? options.resolve.external : parserOptions.resolve.external,
      file: fileOption,
      http: {
        ...typeof parserOptions.resolve.http === "object" ? parserOptions.resolve.http : {},
        ...typeof _optionalChain([options, 'optionalAccess', _77 => _77.resolve, 'optionalAccess', _78 => _78.http]) === "object" ? options.resolve.http : {},
        timeout: _optionalChain([options, 'optionalAccess', _79 => _79.resolve, 'optionalAccess', _80 => _80.http]) && "timeout" in options.resolve.http ? options.resolve.http.timeout : 5e3,
        safeUrlResolver: true
      }
    },
    timeoutMs: _optionalChain([options, 'optionalAccess', _81 => _81.timeoutMs])
  };
}
...
function validateSchema(api, options = {}, suppressedInstancePaths = []) {
  ...
  try {
    const errors = _betterajverrors2.default.call(void 0, schema, api, reducedErrors, {
      format: "cli-array",
      colorize: _optionalChain([options, 'optionalAccess', _82 => _82.validate, 'optionalAccess', _83 => _83.errors, 'optionalAccess', _84 => _84.colorize]) || false,
      indent: 2
    });
    return {
      valid: false,
...
var SpecificationValidator = (_class13 = class {constructor() { _class13.prototype.__init25.call(this);_class13.prototype.__init26.call(this);_class13.prototype.__init27.call(this); }
  __init25() {this.errors = []}
  __init26() {this.warnings = []}
  /**
   ...
   */
  __init27() {this.flaggedInstancePaths = []}
  reportError(message) {
...
var OpenAPISpecificationValidator = class extends SpecificationValidator {
  ...
  checkSecuritySchemes() {
    const securitySchemes = _optionalChain([this, 'access', _85 => _85.api, 'access', _86 => _86.components, 'optionalAccess', _87 => _87.securitySchemes]);
    if (!securitySchemes) {
      return;
    }
...
// src/index.ts
function isFilesystemPathSource(api) {
  return typeof api === "string" && !/^https?:\/\//i.test(api);
}
async function parse4(api, options) {
...
async function bundle3(api, options) {
  const args = normalizeArguments(api);
  const parserOptions = convertOptionsForParser(options, {
    allowFileResolution: isFilesystemPathSource(api)
  });
...
async function dereference3(api, options) {
  const args = normalizeArguments(api);
  const parserOptions = convertOptionsForParser(options, {
    allowFileResolution: isFilesystemPathSource(api)
  });
...
async function validate(api, options) {
  ...
  let result;
  const circular$RefOption = _optionalChain([parserOptions, 'access', _88 => _88.dereference, 'optionalAccess', _89 => _89.circular]);
  ...
  const openapiRules = _optionalChain([options, 'optionalAccess', _90 => _90.validate, 'optionalAccess', _91 => _91.rules, 'optionalAccess', _92 => _92.openapi]);
  const swaggerRules = _optionalChain([options, 'optionalAccess', _93 => _93.validate, 'optionalAccess', _94 => _94.rules, 'optionalAccess', _95 => _95.swagger]);
  const rules = {
    openapi: {
      "array-without-items": _optionalChain([openapiRules, 'optionalAccess', _96 => _96["array-without-items"]]) || "error",
      "duplicate-non-request-body-parameters": _optionalChain([openapiRules, 'optionalAccess', _97 => _97["duplicate-non-request-body-parameters"]]) || "error",
      "duplicate-operation-id": _optionalChain([openapiRules, 'optionalAccess', _98 => _98["duplicate-operation-id"]]) || "error",
      "invalid-security-scheme-properties": _optionalChain([openapiRules, 'optionalAccess', _99 => _99["invalid-security-scheme-properties"]]) || "error",
      "non-optional-path-parameters": _optionalChain([openapiRules, 'optionalAccess', _100 => _100["non-optional-path-parameters"]]) || "error",
      "path-parameters-not-in-parameters": _optionalChain([openapiRules, 'optionalAccess', _101 => _101["path-parameters-not-in-parameters"]]) || "error",
      "path-parameters-not-in-path": _optionalChain([openapiRules, 'optionalAccess', _102 => _102["path-parameters-not-in-path"]]) || "error"
    },
    swagger: {
      "array-without-items": _optionalChain([swaggerRules, 'optionalAccess', _103 => _103["array-without-items"]]) || "error",
      "duplicate-non-request-body-parameters": _optionalChain([swaggerRules, 'optionalAccess', _104 => _104["duplicate-non-request-body-parameters"]]) || "error",
      "duplicate-operation-id": _optionalChain([swaggerRules, 'optionalAccess', _105 => _105["duplicate-operation-id"]]) || "error",
      "invalid-security-scheme-properties": _optionalChain([swaggerRules, 'optionalAccess', _106 => _106["invalid-security-scheme-properties"]]) || "error",
      "non-optional-path-parameters": _optionalChain([swaggerRules, 'optionalAccess', _107 => _107["non-optional-path-parameters"]]) || "error",
      "path-parameters-not-in-parameters": _optionalChain([swaggerRules, 'optionalAccess', _108 => _108["path-parameters-not-in-parameters"]]) || "error",
      "path-parameters-not-in-path": _optionalChain([swaggerRules, 'optionalAccess', _109 => _109["path-parameters-not-in-path"]]) || "error",
      "unknown-required-schema-property": _optionalChain([swaggerRules, 'optionalAccess', _110 => _110["unknown-required-schema-property"]]) || "error"
    }
  };
  ...
  if (_optionalChain([parser, 'access', _111 => _111.$refs, 'optionalAccess', _112 => _112.circular])) {
...
exports.MissingPointerError = MissingPointerError; exports.ResolverError = ResolverError; exports.bundle = bundle3; exports.compileErrors = compileErrors; exports.dereference = dereference3; exports.parse = parse4; exports.validate = validate;
