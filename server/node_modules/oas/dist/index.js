...
var Oas = class _Oas {
  ...
  findRootServerOperationMatches(url) {
    ...
    let target = substituted?.pathName ? substituted : void 0;
    if (!target) {
    ...
    Object.keys(paths).forEach((path) => {
      ...
      if (!mayHaveOverrides) {
        for (const method of supportedMethods) {
          ...
          if (rawOperation && (isRef(rawOperation) || rawOperation.servers?.length)) {
      ...
      supportedMethods.forEach((method) => {
        ...
        const servers = operation.servers?.length ? operation.servers : pathItem.servers?.length ? pathItem.servers : void 0;
        if (!servers) return;
