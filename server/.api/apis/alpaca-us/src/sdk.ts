...
export default class SDK {
  ...
  constructor() {
    this.core = new APICore(definition, 'alpaca-us/1.0.0 (api/7.0.0)');
  }
   ...
   * - `urn:ietf:params:oauth:grant-type:jwt-bearer`: Exchange JWT assertion (Keycloak for
   * brokerdash or Google for admin)
   * - `refresh_token`: Refresh an admin access token using a refresh token
   *
   *
   * @summary Issue tokens
   */
  issueTokens(body: types.TokenRequest): Promise<FetchResponse<200, types.TokenResponse> | FetchResponse<number, types.ErrorResponse>> {
